...
if (isset($_GET['img'])) {
    $requestedRaw = $_GET['img'];
    $requested = rawurldecode($requestedRaw);
    ...
    // Patching LFI to only render images
    $allowed_extension_images = ['jpg', 'jpeg', 'png'];

    if (!in_array($ext, $allowed_extension_images, true)) {
        die('NICE TRY!');
    } else {
        $finfoType = null;
        if (function_exists('finfo_open')) {
            $finfo = finfo_open(FILEINFO_MIME_TYPE);
            $finfoType = finfo_file($finfo, $real);
            finfo_close($finfo);
        } elseif (function_exists('mime_content_type')) {
            $finfoType = mime_content_type($real);
        }
    ...
    /* Patch Rendering of PHP Files  via LFI
    if ($ext === 'php') {

        //ob_start();

        //header('X-LFI-Exec: true');
...
// Patched SQLi Vulnerability

$stmt = $db->prepare('SELECT id, name, filename, uploader FROM cars WHERE id = :id LIMIT 1');
...
if (!$row) {
    http_response_code(404);
    echo '<!doctype html><html><head><meta charset="utf-8"><title>Not found</title><script src="scripts/tailwind.js"></script></head><body class="bg-gray-50">';
...
<head>
  ...
  <style>.card-shadow { box-shadow: 0 6px 18px rgba(15,23,42,0.06); }</style>
</head>
    ...
    <div class="bg-white rounded-2xl p-6 card-shadow">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
        ...
        <div>
          ...
          <!--Patching XSS vuln-->
          <p class="text-sm text-gray-500 mb-2">Uploaded by: <strong><?php echo htmlspecialchars($row['uploader'], ENT_QUOTES, 'UTF-8'); ?></strong></p>
